...
// 이제 다음과 같은 동작을 카드가 한 장 남을 때까지 반복하게 된다. 우선, 제일 위에 있는 카드를 바닥에 버린다. 그 다음, 제일 위에 있는 카드를 제일 아래에
// 있는 카드 밑으로 옮긴다.
...
public class Main2164 {
    ...
    public static void main(String args[]) throws IOException{
        ...
        while(myQueue.size()>1){
            myQueue.poll();    // 선 삭제
            myQueue.add(myQueue.poll());    // 후에 추가
            System.out.println(myQueue);
        }
        bw.write(String.valueOf(myQueue.peek()));
        bw.close();
    }
}
